...
montserradoBox <- create_buffer(x = montserradoBox, buffer = 2, country = "Liberia")

x <- seq(from = extent(montserradoBox)[1], to = extent(montserradoBox)[2], by = 30000)

y <- seq(from = extent(montserradoBox)[3], to = extent(montserradoBox)[4], by = 17320.51)
...
montserradoGrid <- coords %>% 
  SpatialPoints(proj4string = CRS(as.character(map_projections$proj[map_projections$country == "Liberia"]))) %>%
  SpatialPixels()
