...
\begin{longtable}[]{@{}rl@{}}
\caption{\label{tab:survey2} Scenario 1 - coverage survey results from a
...
\label{tab:survey1}Scenario 1: coverage survey results from a programme with
good case-finding and recruitment and short lengths of stay

Number of current cases

2

Number of current cases in the program

0

Number of current cases not in the program

2

Number of recovering cases in the program

34

In this regard, the period coverage most likely better depicts the
coverage of the programme. On the other hand, a programme with poor
case-finding and recruitment and long lengths of stay may have a period
coverage that is misleadingly high because of high number of recovering
cases when in fact point coverage is the more reflective coverage of the
programme. In addition, period coverage suffers from an overestimation
bias. This is because the current period coverage estimator does not
take into account cases of acute malnutrition who have recovered
spontaneously but were never enrolled in any treatment programme.

A recent Epicentre review \citep{Epicentre:2015ty} has highlighted the
existence of these two coverage estimators as a source of confusion,
with the risk that implementers may choose to assess period coverage
(rather than point) simply because it produces a higher estimate. The
review suggests that both coverage indicators could be reported, with
sufficient context (e.g.~on length of stay, timeliness of admissions,
etc.) to allow for their interpretation. For this reason work has been
conducted on further developing the coverage estimators to address this
confusion around them. This work has included:

\begin{itemize}
\item
  A shift in terminology that is more descriptive and specific with
  regard to what the estimator is actually measuring, allowing both
  measures to be reported together without confusion. \textbf{Point
  coverage} is now named \emph{case-finding effectiveness} to more
  precisely reflect it as a measure of the programme's ability to find
  and recruit current cases. This indicator assesses how good the
  treatment programme is in finding cases of MAM and then getting them
  to treatment. A programme with effective case-finding will always have
  good overall coverage when this estimator is used. \textbf{Period
  coverage} is now named \emph{treatment coverage} as this is the
  estimator that approximates this coverage indicator the closest.
\item
  Improvement of the period coverage estimator so that it can be used
  more precisely as a single coverage estimator for treatment coverage.
\end{itemize}

We propose therefore that programmes report both case-finding
effectiveness and the improved treatment coverage indicator and,
crucially, give sufficient context to properly evaluate both estimates.
For a more detailed description of the definitions of these two
estimators, their differences and limitations, and the development work
completed under this project, see Annex 3.
