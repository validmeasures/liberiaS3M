...
need to be sampled to obtain the target sample size of 96 SAM children
in each county (and urban areas if included). This can be estimated as
follows:
...
Liberia Institute of Statistics and Geo-information Services to UNICEF.
...
\end{minipage} & \begin{minipage}[b]{0.20\columnwidth}\centering
\textbf{Length of survey} (per county/area)\strut
\end{minipage} & \begin{minipage}[b]{0.20\columnwidth}\centering
\textbf{Length of survey} (2 counties)\strut
\end{minipage} & \begin{minipage}[b]{0.20\columnwidth}\centering
\textbf{Length of survey} (2 rural, 1 urban)\strut
\end{minipage}\tabularnewline
\midrule
\endfirsthead
\toprule
...
\[\begin{aligned}
\hat{v} & ~ = ~ \frac{\sum_{i = 1}^{n} \frac{1}{d_{i}^{p}}v_{i}}{\sum_{i = 1}^{n}\frac{1}{d_{i}^{p}}} \\
\\
where: & \\
\\
d_1 \ldots d_n  & ~ = ~ \text{distances from each of the } n \text{ sampling point locations to the point being estimated} \\
